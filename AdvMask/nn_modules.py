...
class FaceXZooProjector(nn.Module):
    ...
    def align_patch(self, adv_patch, landmarks):
        ...
        # print(grid.size())
        cropped_image = kornia.geometry.remap(cropped_image, map_x=grid[..., 0], map_y=grid[..., 1], mode='nearest')
        # save_image(cropped_image, "outputs/cropped.png")
...
class PRN:
    ...
    def preprocess(self, img_batch, image_info):
        ...
        # wrap
        tform = kornia.geometry.transform.imgwarp.get_perspective_transform(src_pts, dst_pts)
        cropped_image = kornia.geometry.warp_perspective(img_batch, tform, dsize=(self.resolution, self.resolution))
        # save_image(cropped_image, "outputs/PRNcrop.png")
